...
为了扩展 Gemini CLI 的功能，可以通过创建 TOML 文件来自定义斜杠命令。

-   **命令文件位置**:
    -   **全局命令**: `~/.gemini/commands/` (在所有项目中可用)
    -   **项目命令**: `<project-root>/.gemini/commands/` (项目特定，可版本控制，会覆盖同名全局命令)

-   **TOML 文件格式**:
    -   **必需字段**: `prompt = """您的多行提示模板"""` (可包含 `{{args}}`, `!{shell}`, `@{file}` 语法)
    -   **可选字段**: `description = "在 /help 菜单中显示的简短描述"`

-   **命名规则**: 文件路径决定命令名称。
    -   示例: `~/.gemini/commands/test.toml` → `/test`
    -   示例: `<project>/.gemini/commands/git/commit.toml` → `/git:commit`

-   **参数处理**:
    -   **显式参数注入**: 提示中包含 `{{args}}` 时，会被用户提供的参数替换。
    -   **默认参数追加**: 提示中不包含 `{{args}}` 时，CLI 会将完整的命令调用追加到提示后。

-   **高级功能**:
    -   **Shell 命令执行**: 使用 `!{...}` 语法执行 shell 命令并注入输出。
        -   **注意**: Shell 命令会触发安全确认对话框。
    -   **文件内容注入**: 使用 `@{...}` 语法注入文件内容。支持文本文件、图片 (PNG, JPEG)、PDF、音频和视频。
